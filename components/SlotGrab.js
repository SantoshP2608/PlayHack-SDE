      ...
      <main id="main" className="mx-auto w-full max-w-6xl px-5 py-8 sm:px-8 lg:px-10">
        {view === "explore" ? <>
          <section className="hero">
            <div><span className="hero-campus">IIT GUWAHATI <span>· CAMPUS SPORTS</span></span><h1>Your next game<br />starts here.</h1><p>Made for the IIT Guwahati community.<br />Your campus. Your court. Your time to play.</p></div>
            <CourtIllustration />
          </section>
          ...
          <div className="grid items-start gap-7 lg:grid-cols-[1.08fr_1fr]">
            <section aria-labelledby="facilities-heading">
              <div className="section-title"><h2 id="facilities-heading">Find your court</h2><span>{courtCounts[selection.sport]} courts</span></div>
              {Array.from({ length: courtCounts[selection.sport] }, (_, index) => index + 1).map((court) => {
                ...
                return <button key={court} className="court-button" disabled={busy} aria-pressed={selection.court === court} onClick={() => choose({ court })}>
                  <span className="court-number">0{court}</span><span className="min-w-0"><strong>{selection.sport} Court {court}</strong><small>Whole court · 60-minute session</small></span><span className="availability">{free} times available <span aria-hidden="true">↗</span></span>
                </button>;
              })}
            ...
            <section className="booking-panel" aria-labelledby="session-heading" aria-busy={busy}>
              ...
              <h2 id="session-heading">{selection.sport} Court {selection.court}</h2><p className="text-sm">Whole court · One booking per hour</p>
              ...
              <div className="selection-summary" data-status={status} aria-live="polite"><strong>{day.full} · {slots[selection.time].range}</strong><span>{status === "yours" ? "This court is reserved for you." : status === "waiting" ? `Waitlist position #${wait.position} · No booking yet.` : status === "booked" ? "Booked · Join the first-come waitlist." : "Available · Reserve the whole court for this hour."}</span></div>
